...
public class WallJump : MonoBehaviour
{
    ...
    public Vector2? GetWallNormal()
    {
        ...
        if (hit = Physics2D.Raycast(transform.position, Vector2.right, _collider.bounds.size.x / 2f + CHECK_DISTANCE, CommonExtensions.DefaultGroundMask))
            return hit.normal;

        //Checks for left wall
        if (hit = Physics2D.Raycast(transform.position, Vector2.left, _collider.bounds.size.x / 2f + CHECK_DISTANCE, CommonExtensions.DefaultGroundMask))
            return hit.normal;
