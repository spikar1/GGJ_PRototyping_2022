...
namespace DigitalRuby.AdvancedPolygonCollider
{
    public sealed class TextureConverter
    {
        ...
        public List<Vertices> DetectVertices(UnityEngine.Color[] colors, int width, int alphaTolerance)
        {
            ...
            coloredSolids = new int[colors.Length];
            ...
            var neighbors = new int[]{-1, 1, width, -width};
            var floatolors = 0;
            for (int i = 0; i< solids.Length; i++)
            {
                ...
                // Flood fill the area with the color
                var stack = new Stack<int>();
                stack.Push(i);
                while (stack.Count > 0)
                {
                    var index = stack.Pop();
                    coloredSolids[index] = floatolors;
                    for (int j = 0; j < neighbors.Length; j++)
                    {
                        var neighbor = index + neighbors[j];
                        if (neighbor < 0 || neighbor >= solids.Length) continue;
                        if (coloredSolids[neighbor] != 0) continue;
                        if (solids[neighbor] != 0) continue;
                        stack.Push(neighbor);
                    }
        ...
        private bool InPolygon(Vertices polygon, Vector2 point)
        {
            ...
            var polygonColor = coloredSolids[(int)polygon[0].x + (int)polygon[0].y * width];
            var pointColor = coloredSolids[(int)point.x + (int)point.y * width];
            return polygonColor == pointColor;
